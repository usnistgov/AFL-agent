...
try:
    ...
    warnings.warn("Dirchlet-GP Extrapolator requires gpytorch."
                  "Install it from here : https://github.com/cornellius-gp/gpytorch")
...
class DirichletGPExtrapolator(Extrapolator):
    ...
    def calculate(self, dataset: xr.Dataset) -> Self:
        ...
        if len(np.unique(y)) == 1:
            ...
            train_y = torch.Tensor(y).long().squeeze()
            likelihood = DirichletClassificationLikelihood(train_y, learn_additional_noise=True)
            model = DirichletGPModel(y,
            ...
            self.output[self._prefix_output("y_prob")] = xr.DataArray(
                probabilities, dims=self.grid_dim
            )
    ...
    def fit(self, train_x, model, likelihood, **kwargs):
        ...
        for i in range(kwargs.get("training_iter" , 200)):
            optimizer.zero_grad()
