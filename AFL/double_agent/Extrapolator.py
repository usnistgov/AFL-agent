...
class GaussianProcessRegressor(Extrapolator):
    ...
    def calculate(self, dataset):
        ...
        if self.predictor_uncertainty_variable != None:
            ...
            reg = sklearn.gaussian_process.GaussianProcessRegressor(kernel=self.kernel, alpha=dy.values).fit(X.values, y.values)
            reg_type = "homoscedastic"
        else:
            reg = sklearn.gaussian_process.GaussianProcessRegressor(kernel=self.kernel).fit(X.values, y.values)
